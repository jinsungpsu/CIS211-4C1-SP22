...
//        return neighborsArr;
    ...
    public void depthFirstTraversal(T src) {
        ...
        // this implementation used integers
        // had to make changes to make it generic
        // by using our hash maps

        // ArrayList<T> visited = new ArrayList<>();
        // this would have been very inefficient
        // because we would have had to do a linear search
        // every time we wanted to check if a vertex was visited

        boolean[] visited = new boolean[adjList.length];
        // an array of booleans is much more efficient!

        Stack<T> verticesStack = new Stack<>();

        verticesStack.push(src);
        visited[vertices.get(src)] = true;;

        // loop, is my stack empty?
        while(!verticesStack.isEmpty()) {

            T whereAmINow = verticesStack.pop();

            if (visited[vertices.get(whereAmINow)] == false) {
                System.out.println("I am at: " + whereAmINow);
                visited[vertices.get(whereAmINow)] = true;
            }

            int index = vertices.get(whereAmINow);
            LinkedList<T> neighbors = adjList[index];

            for (int i = 0; i < neighbors.size(); i++) {
                T neighbor = neighbors.get(i);
                int indexOfNeighbor = vertices.get(neighbor);
                if (visited[indexOfNeighbor] == false) {
